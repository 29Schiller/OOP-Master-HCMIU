...
public class Pea extends Plants{
    ...
    public void importImage() {
        for (int i = 0; i < PeaNoActionImage.length; i++) {
            try {PeaNoActionImage[i] = ImageIO.read(getClass().getResourceAsStream("/Resource/Plants/Pea/PeaNoAction/" + (i + 1) + ".png"));
            } catch (IOException e) {e.printStackTrace();}}
        for (int i = 0; i < PeaActionImage.length; i++) {
            try {PeaActionImage[i] = ImageIO.read(getClass().getResourceAsStream("/Resource/Plants/Pea/PeaAction/" + (i + 1) + ".png"));
            } catch (IOException e) {e.printStackTrace();}}
        try {PeaBulletImage = ImageIO.read(getClass().getResourceAsStream("/Resource/Plants/Pea/Bullet/Pea.png"));
        } catch (IOException e) {e.printStackTrace();}
    }
